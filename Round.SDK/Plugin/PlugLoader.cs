...
public class PlugLoader
{
    ...
    {
        if (IsLoaded) throw new InvalidOperationException("该PlugLoader实例已加载过插件包，请创建新的实例来加载其他插件包");

        if (string.IsNullOrEmpty(pluginPackagePath)) throw new ArgumentException("插件包路径不能为空");

        if (!File.Exists(pluginPackagePath)) throw new FileNotFoundException($"插件包文件不存在: {pluginPackagePath}");
        ...
        try
        {
            ...
            // 1. 解压插件包
            var extractDir = Path.Combine(ExtractPath,
    ...
    {
        if (!IsLoaded) throw new InvalidOperationException("插件尚未加载，请先调用Load方法");
    ...
    {
        if (!IsLoaded || PluginInstance == null) throw new InvalidOperationException("插件尚未加载，请先调用Load方法");
